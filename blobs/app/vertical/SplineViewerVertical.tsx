      ...
      <div className="left-panel">
        ...
        {isConnected && (
          <button
            ...
            onClick={async () => {
              console.log("🔴 結束對話按鈕被點擊");
              console.log("📊 當前狀態:", {
                isConnected,
                showChat,
                chatHistoryLength: chatHistory.length,
                currentMessage,
                hasReceivedFirstAgentMessage
              });

              // 先關閉歷史對話區塊
              setShowChat(false);
              console.log("✅ 已設定 showChat = false");

              // 清空歷史對話紀錄
              if (chatManagerRef.current) {
                console.log("🗑️ 正在清空歷史對話...");
                chatManagerRef.current.clearHistory();
                console.log("✅ clearHistory() 已執行");
              } else {
                console.warn("⚠️ chatManagerRef.current 不存在");
              }

              // 重置訊息氣泡
              setCurrentMessage(INITIAL_BUBBLE_MESSAGE);
              console.log("✅ 已重置訊息氣泡");
